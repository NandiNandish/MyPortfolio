...
class ALSpeedCallback extends ALCallbackBase {
	...
	public function clearCacheUrl($purge_url) {
		if (isset($purge_url)) {
			...
			if (!is_array($parsed_url) || empty($parsed_url["host"]) || empty($parsed_url["path"]) ||
	...
	private function isValidCachePath($path) {
		return is_string($path) && $path !== '' && strpos($path, "\0") === false && strpos($path, '..') === false;
	}
	...
	private function deleteAirliftCachePath($cache_base_path, $delete_path) {
		$normalized_base_path = $this->normalizeCachePath($cache_base_path);
		$normalized_delete_path = $this->normalizeCachePath($delete_path);
		if (strpos($normalized_delete_path, $normalized_base_path) !== 0) {
			return false;
		}
	...
	private function deleteExactAirliftCacheFiles($cache_base_path, $delete_path) {
		$normalized_base_path = $this->normalizeCachePath($cache_base_path);
		$normalized_delete_path = $this->normalizeCachePath($delete_path);
		if (strpos($normalized_delete_path, $normalized_base_path) !== 0 || !is_dir($delete_path)) {
			return false;
		}
