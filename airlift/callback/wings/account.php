...
if (!class_exists('ALAccountCallback')) :
class ALAccountCallback extends ALCallbackBase {
	public $account;
	public $settings;
	...
	function updateInfo($args) {
		...
		if (array_key_exists('update_info', $args)) {
			...
			$result['update_info'] = array(
				"status" => ALAccount::exists($this->settings, $args['update_info']['pubkey'])
			);
		}
	
		if (array_key_exists('update_api_key', $args)) {
			ALAccount::updateApiPublicKey($this->settings, $args['update_api_key']['pubkey']);
			$result['update_api_key'] = array(
				"status" => $this->settings->getOption(ALAccount::$api_public_key)
			);
		}
	...
	function process($request) {
		...
		case "addacc":
			ALAccount::addAccount($settings, $params['public'], $params['secret']);
			$resp = array("status" => ALAccount::exists($settings, $params['public']));
			break;
		case "rmacc":
			$resp = array("status" => ALAccount::remove($settings, $params['public']));
			break;
		case "updt":
			$account->updateInfo($params);
			$resp = array("status" => ALAccount::exists($settings, $params['pubkey']));
			break;
		case "updtapikey":
			ALAccount::updateApiPublicKey($settings, $params['pubkey']);
			$resp = array("status" => $settings->getOption(ALAccount::$api_public_key));
			break;
		case "rmbvscrt":
			$resp = array("status" => ALRecover::deleteDefaultSecret($settings));
			break;
		case "rmbvkeys":
		...
		case "rmoldbvacc":
			...
			break;
		case "fetch":
			$accounts = ALAccount::allAccounts($settings);
