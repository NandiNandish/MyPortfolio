...
if (!class_exists('ALOptimizer')) :
	...
	class ALOptimizer {
		...
		public function optimizeBuffer($buffer) {
			...
			if (is_array($template_config) && !empty($template_config)) {
				...
				if (array_key_exists("footer_message", $template_config)) {
					...
					$buffer = $buffer . '<!-- ' . $template_config["footer_message"] . ', Cached Timestamp: ' . $currentTime . ' UTC, Optimization Time: ' . $timeTaken . 'ms -->';
				}
			} else {
				if (array_key_exists("footer_message", $this->config)) {
					$currentTime = gmdate("Y-m-d H:i:s");
					$timeTaken = round((microtime(true) - $startTime) * 1000, 2);
					$buffer = $buffer . '<!-- ' . $this->config["footer_message"] . ', Cached Timestamp: ' . $currentTime . ' UTC, Optimization Time: ' . $timeTaken . 'ms -->';
				}
			}
			return $buffer;
		}
			...
			if ($this->canLazyload($image, $image_config)) {
				if (ALHelper::safePregMatch('#\ssrc\s*=\s*(\'|")(?<src>.*)\1#Uis', $image, $matches)) {
					if (!empty($matches['src'])) {
						...
						if (($width <= 0 && $height <= 0) && $this->shouldUseImageDimensionsForLazyloadPlaceholder($image_config)) {
							$dimensions = $this->getImageDimensionsForPlaceholder($matches['src']);
							if ($dimensions) {
								$width = $dimensions[0];
								$height = $dimensions[1];
		...
		public function hydrateViewportImage($image, $image_config) {
			...
			$width = $this->imageWidth($final_image);
			$height = $this->imageHeight($final_image);
			$attr_string = '';
